...
Երբ նորից կառուցում եմ `basic-s` մոդուլը ու դրա մուտքին տալիս եմ տեստային ծրագիրը, տեսնում եմ, որ սխալի մասին հաղորդագրության մեջ հիմա արդեն նշված է շարահյուսական սխալը պարունակող տողը։

````
$ ./basic-s < case01.bas
ՍԽԱԼ։ [2] syntax error, unexpected xEol, expecting xDeclare or xFunction or xEof
````

Հիմա սխալի մասին։ Բանն այն է, որ ըստ սահմանված քերականության, բեյսիկ ծրագիրը կարող է սկսվել կամ `DECLARE` ու `FUNCTION` ծառայողական բառերով, կամ էլ, եթե ֆայլը միայն բացատանիշեր ու մեկնաբանություններ է պարունակում, ֆայլի վերջը ցույց տվող `xEof` թոքենով։ Դա պարզ երևում է հետևյալ կանոններից․
...
Որպեսզի վերլուծիչը կարողանա տեսնել ու անտեսել ֆայլի սկզբում հանդիպող նոր տողի նիշերը (քանի որ մեկնաբանություններն ու բացատանիշերը անտեսվում են բառային վերլուծիչի կողմից), `Program` կանոնում պետք է ավելացնել մի նոր տարր․
...
Հիմա էլ, երբ փորձում եմ նորից վերլուծիչի մուտքին տալ տեստային ֆայլը, ստանում եմ մի ուրիշ սխալի հաղորդագրություն, որում շարահյուսական վերլուծիչը «բողոքում» է, որ ֆայլի յոթերորդ տողում սպասում էր `$end` հատուկ սիմվոլին, բայց ստացել է `xEof` տերմինալային սիմվոլը․

````
$ ./basic-s < ../tests/case01.bas 
ՍԽԱԼ։ [7] syntax error, unexpected xEof, expecting $end
````
